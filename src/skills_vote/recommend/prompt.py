...
PromptKey = Literal["user_prompt", "system_prompt", "skills_vote_library_system_prompt"]
...
_SKILLS_VOTE_LIBRARY_SYSTEM_PROMPT = """
## TODO

Given the current user query and the candidate skill markdown files under `skills_root`, search and recommend Agent Skills that can help the downstream agent, and generate optimized context as the usage of skills.
...
skills_root/
    ...
    ├── DevOps/
    ├── AgentMeta/
    ├── ProfessionalDomainKnowledge/
    ├── NaturalScienceKnowledge/
    └── Tools/
```

The meanings of the category names are as follows:
...
- `skill_names` (`list[str]`): A list of recommended skill names. Each name must exactly match a real skill markdown file under `skills_root/<category>`. No duplicates are allowed.
- `optimized_context`: (`str`): Concise skill-use guidance for the downstream agent.

Returning an empty `skill_names` list is allowed only after meaningful search and reasoning shows that the current `skills_root` does not contain a relevant or reusable skill markdown file for the requirement.
...
2. Generalize the requirement into multiple search keyword families before selecting skills:
    ...
    - For each core step, consider whether a domain-specific skill, a tooling skill, or a generic workflow skill could help.
3. Use filesystem tools for candidate discovery:
    - Use `find` to inspect candidate markdown files under `skills_root/<category>`.
    - Use `rg` to search markdown content for keywords.
    - Do not rely only on file names , category names, or descriptions.
    - Run additional searches when initial results are sparse, ambiguous, overly literal, or do not cover all core steps.
    - Prefer parallel tool calls for independent search queries.
4. Read candidates selectively but sufficiently:
    - Prefer reading candidate skills that appear relevant from markdown content, search results, paths, names, categories, descriptions, or keywords.
    - For large files, read only the sections directly relevant to capability assessment.
...
5. Iterate search and verification:
    ...
    - If several skills appear similar, read enough information to compare coverage, overlap, and intended usage.
    - Do not call stop before either selecting relevant skills or concluding, with specific evidence, that no relevant skill files exists.
    - Stop searching when the selected skills cover the main steps, or when further searching is unlikely to change the recommendation.
...
- Return an empty list only when you are confident, after content search and candidate reading, that no current skill would help the downstream agent in a meaningful way.
- Do not recommend unrelated skills just to fill `top_k`.
- Do not recommend a skill based only on name similarity if its markdown content does not provide capability evidence.
...
- output the final answer or deliverable for the user's task;
- include detailed search traces, hidden reasoning, or unrelated explanation;
- copy long passages from skill markdown file;
- make unsupported claims about skills that were not read or lack evidence.

## Constraint

- Search and read only files inside `skills_root`.
- Recommend only real markdown files under `skills_root/<category>`.
- Do not invent, rename, synthesize, or infer non-existent skills.
- Do not access files, directories, or paths outside `skills_root`.
- Do not follow or use symlinks, relative paths, or references that resolve outside `skills_root`.
- Do not directly complete the task described in `user_query`.
...
def build_skills_vote_library_system_prompt(**kwargs: Any) -> str:
    return _SKILLS_VOTE_LIBRARY_SYSTEM_PROMPT.format(**kwargs)


def build(key: PromptKey, **kwargs: Any) -> dict[str, Any]:
    if key == "user_prompt":
        return {"user_prompt": build_user_prompt(**kwargs)}
    if key == "skills_vote_library_system_prompt":
        return {
            "skills_vote_library_system_prompt": (
                build_skills_vote_library_system_prompt(**kwargs)
            )
        }
