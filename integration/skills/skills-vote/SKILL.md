...
- `query` (`str`): A standalone and explicit description of the task, suitable for use as an independent skill-recommendation prompt. You may rewrite the user's original request to improve clarity, specificity, and retrieval performance, and may add reasonable implied details or likely substeps when helpful.
